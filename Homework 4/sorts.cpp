...
bool isSorted(const vector<Sensor>& s)
{
    ...
    {
        if (compareSensor(s[k],s[k-1])) // reverse, so if true, means false
            return false;
    }
    return true; // else, means true
}
